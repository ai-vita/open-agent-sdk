...
async function main() {
  ...
  for await (const event of agent.stream("List the files in the current directory and summarize what you see.")) {
    switch (event.type) {
      case "text-delta":
  ...
  for await (const event of agent.stream("Which of those files is the main entry point? Read it and explain what it does.")) {
    switch (event.type) {
      case "text-delta":
        process.stdout.write(event.delta);
        break;
