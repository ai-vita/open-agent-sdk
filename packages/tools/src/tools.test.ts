...
import type { Sandbox, ExecResult } from "@open-agent-sdk/core";
...
describe("createBashTool", () => {
  it("executes command and returns result", async () => {
    ...
    const result = await tool.execute!({ command: "echo hello", timeout: null, description: null, run_in_background: null }, undefined as never);
  ...
  it("blocks commands containing blocked terms", async () => {
    ...
    const result = await tool.execute!({ command: "rm -rf /", timeout: null, description: null, run_in_background: null }, undefined as never);
  ...
  it("truncates long output", async () => {
    ...
    const result = await tool.execute!({ command: "cmd", timeout: null, description: null, run_in_background: null }, undefined as never);
...
describe("createReadTool", () => {
  it("reads file content with line numbers", async () => {
    ...
    const result = await tool.execute!({ file_path: "/test.txt", offset: null, limit: null }, undefined as never);
    const output = result as { type: string; lines: Array<{ line_number: number; content: string }> };
  ...
  it("returns error for non-existent file", async () => {
    ...
    const tool = createReadTool(sandbox);
    const result = await tool.execute!({ file_path: "/ghost.txt", offset: null, limit: null }, undefined as never);
  ...
  it("reads directory entries", async () => {
    const sandbox = createMockSandbox({
      ...
      readDir: vi.fn(async () => [{ name: "a.txt", isDirectory: false }, { name: "b/", isDirectory: true }]),
    });
    const tool = createReadTool(sandbox);
    const result = await tool.execute!({ file_path: "/dir", offset: null, limit: null }, undefined as never);
  ...
  it("respects offset and limit", async () => {
    ...
    const result = await tool.execute!({ file_path: "/test.txt", offset: 3, limit: 2 }, undefined as never);
...
describe("createWriteTool", () => {
  it("writes content to file", async () => {
    ...
    const result = await tool.execute!({ file_path: "/test.txt", content: "hello" }, undefined as never);
    expect((sandbox.writeFile as ReturnType<typeof vi.fn>)).toHaveBeenCalledWith("/test.txt", "hello");
  ...
  it("returns error for disallowed path", async () => {
    ...
    const result = await tool.execute!({ file_path: "/forbidden/file.txt", content: "x" }, undefined as never);
...
describe("createEditTool", () => {
  it("replaces a string in file", async () => {
    ...
    const result = await tool.execute!({ file_path: "/test.txt", old_string: "world", new_string: "earth", replace_all: null }, undefined as never);
  ...
  it("returns error when old_string not found", async () => {
    ...
    const tool = createEditTool(sandbox);
    const result = await tool.execute!({ file_path: "/test.txt", old_string: "missing", new_string: "new", replace_all: null }, undefined as never);
    expect((result as { error: string }).error).toContain("not found");
  });
  ...
  it("rejects when old and new strings are identical", async () => {
    const sandbox = createMockSandbox();
    const tool = createEditTool(sandbox);
    const result = await tool.execute!({ file_path: "/test.txt", old_string: "same", new_string: "same", replace_all: null }, undefined as never);
...
describe("createAskUserTool", () => {
  it("invokes onQuestion callback and returns answer", async () => {
    ...
    const result = await tool.execute!({ question: "What's up?", options: null }, undefined as never);
